...
typealias DecodingEntry = Pair<List<Set<Char>>, List<Set<Char>>>
...
class Day8 : Day<Int, Int, List<DecodingEntry>>() {
    ...
    init {
        ...
        part2(61229, 986034) {
            sumOf { (sequence, cypher) -> cypher.decode(sequence.getDecoding()).toInt() }
        }
    }
    ...
    private fun Set<Char>.is6(five: Set<Char>, three: Set<Char>) = containsAll(five) && minus(five).size == 1 && !is9(three)
    ...
    private fun List<Set<Char>>.decode(key: Map<Set<Char>, Char>) =
        map { key[it]!! }.joinToString("")
}
