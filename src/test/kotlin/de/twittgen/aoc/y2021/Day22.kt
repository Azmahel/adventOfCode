...
class Day22 : Day<List<Day22.Instruction>>() {
    ...
    override fun String.parse() = mapLines { line ->
        ...
        Instruction(command, Cube(x,y,z))
    }
    ...
    fun List<Instruction>.run(target: Cube = Cube(-50..50)): Int {
        val map = mutableMapOf<Point3D, Command>()
        forEach { instruction ->
            with(instruction) {
                val field = cube.intersect(target)
                field?.xR?.forEach { x->
                    field.yR.forEach { y->
                        field.zR.forEach { z ->
                            map[Point3D(x, y, z)] = command
                        }
                    }
                }
            }
        }
        return map.values.count { it == ON }
    }

    private fun List<Instruction>.run2(cubes: List<Pair<Int, Cube>> = emptyList()): Long {
        if( this.isEmpty()) return cubes.sumOf { it.first * it.second.volume }
        val next = first()
        val toToggle = cubes.mapNotNull { existing ->
            // A∪B       =  A + B + -A∩B
            // (A∪B)∪C   = (A + B + -A∩B) + -A∩C + -B∩C + --(A∩B)∩C
            //... (see https://en.wikipedia.org/wiki/Inclusion-exclusion_principle)
             next.cube.intersect(existing.second)?.let { intersect ->
                 -existing.first to intersect
             }
        }.let { if(next.command == ON) it + (1 to next.cube) else it } //Only add new Full set if that area is actually turned on
        return drop(1).run2(cubes + toToggle)
    }

    enum class Command { ON, OFF; }
    data class Instruction(val command: Command, val cube: Cube)
    data class Cube( val xR: IntRange, val yR: IntRange = xR, val zR: IntRange= yR) {
